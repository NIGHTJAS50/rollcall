...
export default function StudentCoursePage() {
  ...
  useEffect(() => {
    fetch(`/api/attendance/${id}`)
      ...
      .then((d) => {
        setData(d);
        const active = d.sessions?.find((s: { isActive: boolean; qrToken: string; id: string }) => s.isActive);
  ...
  async function markAttendance() {
    ...
    if (res.ok) {
      setQrInput("");
      fetch(`/api/attendance/${id}`).then((r) => r.json()).then(setData);
    }
  }
            ...
            {activeSession && (
              <div className="bg-gradient-to-r from-blue-50 to-cyan-50 border border-blue-200 rounded-2xl p-6 mb-6 shadow-sm">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h2 className="font-semibold text-blue-800 text-lg">Active Session — Mark Your Attendance</h2>
                </div>
                <p className="text-sm text-slate-600 mb-4">
                  A session is currently open. Enter the QR token provided by your lecturer to mark yourself present.
                </p>
                ...
                {message && (
                  <p className={`mt-3 text-sm font-medium px-4 py-2.5 rounded-xl ${
              ...
              {!data?.sessions.length ? (
                ...
                <div className="divide-y divide-slate-100">
                  {data.sessions.map((s, i) => (
                    <div key={s.id} className="px-6 py-4 flex items-center justify-between hover:bg-slate-50 transition">
